...
- **18 core Hard Laws** (LAW-01..17 + LAW-CL-95) + **9 v8.8 wiring/self-attestation laws** (LAW-WIRE, LAW-STUB, LAW-SUBSTANCE, LAW-SEED, LAW-AUTH-RT, LAW-ASSERT, LAW-TIER-CLOUD, LAW-WITNESS-TIER, LAW-REVERIFY) + **3 v8.9 behavioral laws** (LAW-RT-PROPAGATE, LAW-CALL, LAW-BEHAVIORAL-MUSTHAVE) + **3 v9.0 laws** (LAW-COV-EXEC coverage-trust, LAW-RT-DERIVE realtime-from-features, LAW-INTENT-DEFAULT archetype default-and-proceed) — inviolable. Violation = atom HALT + automation-ladder demotion.
...
**v9.0 trustworthy-coverage law.** This closes the 2026-06 Facebook-clone field report: the coverage verdict could read 100% (FE *and* BE) on a backend that never booted, because (a) GATE-IMPL certified coverage from agent-self-reported persona status files (no test ever ran), and (b) GATE-10 measured one whole-repo stack with a vacuous-PASS hole on python/go/rust — 100% of zero tests.

**LAW-COV-EXEC COVERAGE ⇒ EXECUTED, NOT SELF-REPORTED** — a coverage PASS MUST trace to a machine-produced coverage report over > 0 instrumented lines; an agent's self-report is a claim, never evidence. Concretely: (a) GATE-IMPL (Stage 4) may not PASS on persona `*-status.json` claims when tests were dispatched against declared `core_flows[]` unless the machine coverage gate (GATE-10) produced a real PASS for that atom — absent / N/A / 0-instrumented machine coverage ⇒ `N/A_PENDING_REVIEWER`, never a self-report PASS; (b) GATE-10 emits `N/A_PENDING_REVIEWER` (not a number) on EVERY stack when 0 statements were instrumented or no report was produced — the node `total==0` guard now also covers python/go/rust, so pytest collecting 0 tests no longer reads as 100%; (c) the mutation gate (GATE-11) — the catcher for assert-nothing tests — may not silently degrade to N/A on a CRITICAL atom (`scale_tier ∈ {scale, hyperscale}`, realtime-class product, or `gates.mechanical.mutation_tool_required=true`) when its tool is absent: there a missing tool is FAIL. Strengthens LAW-11 + LAW-F6: a gate that certifies coverage it never executed is the worst vacuous PASS. Enforced by GATE-IMPL (`scripts/implementer/implementer-coverage-gate.sh`), GATE-10 (`scripts/mechanical/coverage-check.sh`), GATE-11 (`scripts/mechanical/mutation-test.sh`); inversion-tested by `meta/coverage-trust-test.sh`.

**v9.0 realtime-reach.** Same field report: the multi-client proof (GATE-RT-PROPAGATE, v8.9) was correct but never *fired* for the Facebook clone — eligibility keyed on the product NAME (`*chat*|*messag*|…`), and "facebook" / "social-network" matched nothing, so its Messenger feature went N/A and shipped unproven. Separately, implementer personas received no realtime how-to and coded sockets from memory (the repeated WebSocket bugs).

**LAW-RT-DERIVE REALTIME ELIGIBILITY FROM FEATURES, NOT NAME** — GATE-RT-PROPAGATE eligibility is derived from declared `feature_surface[].name` + `core_flows[]` (keywords: chat, messaging, presence, typing, live feed/update/notification, collaborative/co-edit, websocket), in addition to the product-name regex and explicit `realtime.enabled`. A build whose features include a realtime-class capability but leaves `realtime.enabled != true` is FAIL (the proof may not be silently skipped because the product happens to be *named* something the regex missed). Pairs with the implementer **tech-pack** mechanism: when the stack/features declare realtime, the dispatcher MUST hand `references/tech-packs/websocket-realtime.md` (auth-on-upgrade, reconnect+resync, heartbeat, room scoping, idempotent + persisted delivery, optimistic-send reconcile, E2E drivability) to the Dev-Backend + Dev-Frontend personas. Enforced by GATE-RT-PROPAGATE (`scripts/mechanical/e2e-multiclient.sh`); inversion-tested by `meta/realtime-propagation-test.sh` (cases 6–7: feature-derived FAIL + no-false-positive N/A). *Deferred to v9.0.1:* auto-deriving `realtime.{enabled,journeys}` from features (selector inference) — until then the feature-derived FAIL forces the builder to declare the probe config.

**v9.0 intent ergonomics.** Same field report: a bare `build facebook` triggered a fixed 7-field probe plus a mandatory 10–20-item feature-enumeration interview that the vacuous-PASS guard refused to skip ("took all day to answer"). This also exposed a standing conflict — the operating rule "never stop mid-flow to confirm" vs LAW-CL-95 / LAW-INTENT-FS, which *mandate* the interactive feature-confirmation stop.

**LAW-INTENT-DEFAULT KNOWN ARCHETYPE ⇒ DEFAULT-AND-PROCEED** — when the raw prompt matches a curated archetype (`scripts/intent/archetype-defaults.json`) and intent runs WITHOUT `--interactive`, Stage 0.1 pre-fills `declared.*` (product_type, primary_user, core_flows, success_criteria, scale_tier, cost, feature_surface) from the catalog and records an `agent-default-archetype` history entry. That entry satisfies the feature-surface confirmation guard (LAW-INTENT-FS) WITHOUT an interactive round — `build facebook` reaches READY in one pass, zero questions. This is NOT a vacuous PASS: the defaults are an explicit, recorded, user-overridable known-product spec (edit `declared.*` or re-run `--interactive` for the full interview), not a silent empty inference — so LAW-F6 holds in spirit. The catalog is small + high-confidence; an unmatched prompt falls through to the full probe + interview unchanged. This is the sanctioned reconciliation of "never stop mid-flow" with LAW-CL-95: the stop is replaced by recorded, overridable provenance, not skipped silently. Enforced by `scripts/intent/declare-intent.sh`; inversion-tested by `meta/intent-default-proceed-test.sh`. *Deferred to v9.0.1:* broadening the archetype catalog beyond the seed set.
...
- **GATE-10 COVERAGE** — line ≥ T1 AND branch ≥ T2. **v9.0 (LAW-COV-EXEC):** 0 instrumented statements / no report produced ⇒ `N/A_PENDING_REVIEWER` on every stack (node/python/go/rust) — 100%-of-zero-tests is never a PASS. Script: `mechanical/coverage-check.sh`.
