...
| **GATE-RT-PROPAGATE (v8.9; v9.0 feature-derived)** | `scripts/mechanical/e2e-multiclient.sh` | 2 browser contexts: A emits marker → B observes live (no reload) within budget. Realtime-class by product NAME **or declared feature** (chat/messaging/presence/live-notif — LAW-RT-DERIVE) with `realtime.enabled!=true` ⇒ FAIL; enabled+0 journeys ⇒ FAIL; undrivable ⇒ N/A | binary |
| **GATE-CALL (v8.9)** | `scripts/mechanical/e2e-call.sh` | WebRTC: 2 clients, both `RTCPeerConnection.connectionState==connected` + ≥1 live remote track. Mandatory when call declared / chat-app@scale+ / UI exposes call surface. Dead call button ⇒ FAIL; peer accessor absent ⇒ N/A | binary |
